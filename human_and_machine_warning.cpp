...
double human_distance_3dim_before = 0;
...
int main(int argc, char** argv){
    ros::init(argc, argv, "human_and_machine_collision");
    ...
    while (ros::ok())
    {
        ...
        double m_human[3];       
        m_human[0] = human_vertex_1[0] - human_vertex_0[0];
        m_human[1] = human_vertex_1[1] - human_vertex_0[1];
        m_human[2] = human_vertex_1[2] - human_vertex_0[2];
        ...
        double human_distance_3dim;
        human_distance_3dim = calculate_d_min_function(x0_boom_arm, x0_arm_bucket, x0_bucket_end, human_vertex_0, m_boom_arm, m_arm_bucket, m_bucket_end, m_human);
        ...
        if (pow(machine_x - base_x, 2) + pow(machine_y - base_y, 2) <= pow(r_max, 2))
        {
            if (s1>=0 && s1<=1)
            {
                machine_distance = absolute_value_function(t1) * pow((pow(cul_point_y - base_y, 2) + pow(base_x - cul_point_x, 2)), 0.5);
            }
            else
            {
                double end_point1, end_point2;
                ...
                if (end_point1 < end_point2)
                {
                    machine_distance = end_point1;
                }
                else
                {
                    machine_distance = end_point2;
                }
            }
        }
        else
        {
            machine_distance = 10000;
        }
        ...
        // double s1, t1;
        // double d_machine[4];
        // // double machine_vertex_0[2], machine_vertex_1[2], machine_vertex_2[2], machine_vertex_3[2];
        // double px_machine[4], py_machine[4];

        // px_machine[0] = machine_x - 0.5 * machine_lx;
        // px_machine[1] = machine_x + 0.5 * machine_lx;
        // px_machine[2] = machine_x + 0.5 * machine_lx;
        // px_machine[3] = machine_x - 0.5 * machine_lx;

        // py_machine[0] = machine_y - 0.5 * machine_ly;
        // py_machine[1] = machine_y - 0.5 * machine_ly;
        // py_machine[2] = machine_y + 0.5 * machine_ly;
        // py_machine[3] = machine_y + 0.5 * machine_ly;
        ...
        //     if (i < 3)
        //     {
        //         a1 = pow(px_machine[i] - px_machine[i+1], 2) + pow(py_machine[i] - py_machine[i+1], 2);
        //         b1 = 2 * ((px_machine[i] - px_machine[i+1]) * px_machine[i+1] + (py_machine[i] - py_machine[i+1]) * py_machine[i+1]);
        //         c1 = pow(cul_point_x, 2) + pow(cul_point_y, 2);
        //         d1 = -2 * (cul_point_x * px_machine[i+1] + cul_point_y * py_machine[i+1]);
        //         e1 = -2 * ((px_machine[i] - px_machine[i+1]) * cul_point_x + (py_machine[i] - py_machine[i+1]) * cul_point_y);
        //         f1 = pow(px_machine[i+1], 2) + pow(py_machine[i+1], 2);

        //         s1 = (-b1 * e1 + 2 * a1 * d1) / (pow(e1, 2) - 4 * a1 * c1);
        //         t1 = (2 * b1 * c1 - e1 * d1) / (pow(e1, 2) - 4 * a1 * c1);

        //         if (0 <= s1 && s1 <= 1 && 0 <= t1 && t1 <= 1)
        //         {
        //             d_machine[i] = pow((a1 * pow(t1, 2) + b1 * t1 + c1 * pow(s1, 2) + d1 * s1 + e1 * s1 * t1 + f1 + 0.001), 0.5);
        //         }                
        //         else
        //         {
        //             double tmp[4];
        //             tmp[0] = pow((pow(px_machine[i] - cul_point_x, 2) + pow(py_machine[i] - cul_point_y, 2)), 0.5);
        //             tmp[1] = pow((pow(px_machine[i+1] - cul_point_x, 2) + pow(py_machine[i+1] - cul_point_y, 2)), 0.5);
        //             tmp[2] = pow((pow(px_machine[i], 2) + pow(py_machine[i], 2)), 0.5);
        //             tmp[3] = pow((pow(px_machine[i+1], 2) + pow(py_machine[i+1], 2)), 0.5);
        ...
        //     {
        //         // int i = 3;
        //         a1 = pow(px_machine[i] - px_machine[i-3], 2) + pow(py_machine[i] - py_machine[i-3], 2);
        //         b1 = 2 * ((px_machine[i] - px_machine[i-3]) * px_machine[i-3] + (py_machine[i] - py_machine[i-3]) * py_machine[i-3]);
        //         c1 = pow(cul_point_x, 2) + pow(cul_point_y, 2);
        //         d1 = -2 * (cul_point_x * px_machine[i-3] + cul_point_y * py_machine[i-3]);
        //         e1 = -2 * ((px_machine[i] - px_machine[i-3]) * cul_point_x + (py_machine[i] - py_machine[i-3]) * cul_point_y);
        //         f1 = pow(px_machine[i-3], 2) + pow(py_machine[i-3], 2);
        ...
        //             double tmp[4];
        //             tmp[0] = pow((pow(px_machine[i] - cul_point_x, 2) + pow(py_machine[i] - cul_point_y, 2)), 0.5);
        //             tmp[1] = pow((pow(px_machine[i-3] - cul_point_x, 2) + pow(py_machine[i-3] - cul_point_y, 2)), 0.5);
        //             tmp[2] = pow((pow(px_machine[i], 2) + pow(py_machine[i], 2)), 0.5);
        //             tmp[3] = pow((pow(px_machine[i-3], 2) + pow(py_machine[i-3], 2)), 0.5);
        ...
        // machine_distance = d_machine[0];
        // int index_m = 0;
        // for (int i = 0; i < 4; i++)
        // {
        //     if (machine_distance > d_machine[i])
        //     {
        //         machine_distance = d_machine[i];
        //         index_m = i;
        //     }
        ...
        if (human_distance_3dim_before > 1.0 && human_distance_3dim < 1.0)
        {
        ...
        // }
        ROS_INFO("h_d is %f\n", human_distance);
        // ROS_INFO("d_min is %f, %d\n", machine_distance, index_m);
        ROS_INFO("Human is %d, Machine is %d, Human_danger_3dim is %d\n", num_collision_human, num_collision_machine, num_danger);
        // ROS_INFO("machine0 x:%f, y:%f\n", px_machine[0], py_machine[0]);
        // ROS_INFO("machine1 x:%f, y:%f\n", px_machine[1], py_machine[1]);
        // ROS_INFO("machine2 x:%f, y:%f\n", px_machine[2], py_machine[2]);
        // ROS_INFO("machine3 x:%f, y:%f\n", px_machine[3], py_machine[3]);
        ...
        human_distance_3dim_before = human_distance_3dim;
