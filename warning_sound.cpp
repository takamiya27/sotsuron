...
int main(int argc, char** argv){
    ros::init(argc, argv, "warning_sound");
    ...
    ros::Publisher pub_sound = n.advertise<sound_play::SoundRequest>("/robotsound", 10);
   ...
   for (int i = 0; i < 8; i++)
   {
    ...
    while (ros::ok())
    {
        ...
        double s_human, t_human;
        double a_human, b_human, c_human, d_human;

        a_human = cul_point_x - base_x;
        b_human = base_y - cul_point_y;
        c_human = human_x - base_x;
        d_human = human_y - base_y;        

        s_human = (a_human * c_human - b_human * d_human) / (pow(a_human, 2) + pow(b_human, 2));
        t_human = (b_human * c_human + a_human * d_human) / (pow(a_human, 2) + pow(b_human, 2));

        if (s_human >= 0 && s_human <= 1)
        {
            human_distance = absolute_value_function(t_human) * pow((pow(cul_point_y - base_y, 2) + pow(base_x - cul_point_x, 2)), 0.5);
        }
        else
        {
        ...
        double s_machine, t_machine;
        double a_machine, b_machine, c_machine, d_machine;

        a_machine = cul_point_x - base_x;
        b_machine = base_y - cul_point_y;
        c_machine = machine_x - base_x;
        d_machine = machine_y - base_y;        

        s_machine = (a_machine * c_machine - b_machine * d_machine) / (pow(a_machine, 2) + pow(b_machine, 2));
        t_machine = (b_machine * c_machine + a_machine * d_machine) / (pow(a_machine, 2) + pow(b_machine, 2));

        if (s_machine >= 0 && s_machine <= 1)
        {
            machine_distance = absolute_value_function(t_machine) * pow((pow(cul_point_y - base_y, 2) + pow(base_x - cul_point_x, 2)), 0.5);
        }
        else
        {
            double end_point1, end_point2;
    ...
    else
    {
        ...
        if ((human_distance < 3.0 || machine_distance < 3.0 || d_shortest_dump < 1.5) && (cmd_msg.velocity[0]!=0 || cmd_msg.velocity[1]!=0 || cmd_msg.velocity[2]!=0 || cmd_msg.velocity[3]!=0))
        {
            sound_msg.sound = 3;
            sound_msg.command = 1;
            sound_msg.volume = 0.5;
            pub_sound.publish(sound_msg);
        }
        else if ((tmp_z < 0.5) && (a_excavate > tmp_x || tmp_x > b_excavate || c_excavate > tmp_y || tmp_y > d_excavate) && (cmd_msg.velocity[0]!=0 || cmd_msg.velocity[1]!=0 || cmd_msg.velocity[2]!=0 || cmd_msg.velocity[3]!=0))
        {
            sound_msg.sound = 3;
            sound_msg.command = 1;
            sound_msg.volume = 0.5;
