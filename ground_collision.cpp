...
int main(int argc, char** argv){
    ros::init(argc, argv, "ground_collision");
    ...
    while (ros::ok())
    {
        ...
        dump_lx = 4.0; //the x_length of dump trucks
        dump_ly = 5.0; //the y_length of dump trucks
        dump_lz = 2.0;

        double dump_vertex[3][8];        
        dump_vertex[0][0] = dump_x - dump_lx / 2;
        dump_vertex[1][0] = dump_y - dump_ly / 2;
        dump_vertex[2][0] = dump_z - dump_lz / 2;

        dump_vertex[0][1] = dump_x + dump_lx / 2;
        dump_vertex[1][1] = dump_y - dump_ly / 2;
        dump_vertex[2][1] = dump_z - dump_lz / 2;

        dump_vertex[0][2] = dump_x + dump_lx / 2;
        dump_vertex[1][2] = dump_y - dump_ly / 2;
        dump_vertex[2][2] = dump_z + dump_lz / 2;

        dump_vertex[0][3] = dump_x - dump_lx / 2;
        dump_vertex[1][3] = dump_y - dump_ly / 2;
        dump_vertex[2][3] = dump_z + dump_lz / 2;

        dump_vertex[0][4] = dump_x - dump_lx / 2;
        dump_vertex[1][4] = dump_y + dump_ly / 2;
        dump_vertex[2][4] = dump_z - dump_lz / 2;

        dump_vertex[0][5] = dump_x + dump_lx / 2;
        dump_vertex[1][5] = dump_y + dump_ly / 2;
        dump_vertex[2][5] = dump_z - dump_lz / 2;

        dump_vertex[0][6] = dump_x + dump_lx / 2;
        dump_vertex[1][6] = dump_y + dump_ly / 2;
        dump_vertex[2][6] = dump_z + dump_lz / 2;

        dump_vertex[0][7] = dump_x - dump_lx / 2;
        dump_vertex[1][7] = dump_y + dump_ly / 2;
        dump_vertex[2][7] = dump_z + dump_lz / 2;
        ...
        dump_vertex_0[0] = dump_vertex[0][0];
        dump_vertex_0[1] = dump_vertex[1][0];
        dump_vertex_0[2] = dump_vertex[2][0];

        dump_vertex_1[0] = dump_vertex[0][1];
        dump_vertex_1[1] = dump_vertex[1][1];
        dump_vertex_1[2] = dump_vertex[2][1];

        dump_vertex_2[0] = dump_vertex[0][2];
        dump_vertex_2[1] = dump_vertex[1][2];
        dump_vertex_2[2] = dump_vertex[2][2];

        dump_vertex_3[0] = dump_vertex[0][3];
        dump_vertex_3[1] = dump_vertex[1][3];
        dump_vertex_3[2] = dump_vertex[2][3];

        dump_vertex_4[0] = dump_vertex[0][4];
        dump_vertex_4[1] = dump_vertex[1][4];
        dump_vertex_4[2] = dump_vertex[2][4];

        dump_vertex_5[0] = dump_vertex[0][5];
        dump_vertex_5[1] = dump_vertex[1][5];
        dump_vertex_5[2] = dump_vertex[2][5];

        dump_vertex_6[0] = dump_vertex[0][6];
        dump_vertex_6[1] = dump_vertex[1][6];
        dump_vertex_6[2] = dump_vertex[2][6];

        dump_vertex_7[0] = dump_vertex[0][7];
        dump_vertex_7[1] = dump_vertex[1][7];
        dump_vertex_7[2] = dump_vertex[2][7];
        ...
        for (int i = 0; i < 12; i++)
        {
            if ((i >= 0 && i < 3) || (i >= 4 && i < 7))
            {
                for (int j = 0; j < 3; j++)
                {
                    m[j][i] = dump_vertex[j][i+1] - dump_vertex[j][i];
                } 
            }
            else if (i == 3 || i == 7)
            {
                for (int k = 0; k < 3; k++)
                {
                    m[k][i] = dump_vertex[k][i-3] - dump_vertex[k][i];
                }   
            }
            else
            {
                for (int l = 0; l < 3; l++)
                {
                    m[l][i] = dump_vertex[l][i-4] - dump_vertex[l][i-8];
                }
            }           
        }
